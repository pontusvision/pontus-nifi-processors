...
public class PontusMSOfficeTextExtractor extends AbstractProcessor
{
  ...
  public void handleText (ProcessSession session, Metadata metadata, Map<String,String> currAttribs,ContentHandler parserHandler)
      throws SAXException
  {
  ...
  protected class EmbeddedTrackingExtrator extends ParsingEmbeddedDocumentExtractor
  {
        ...
        throws SAXException, IOException
    {
      ...
      handleText(session,metadata,currAttribs,parserHandler);
    }
  }
  ...
  public static final PropertyDescriptor DOCX_FORMAT = new PropertyDescriptor.Builder().name("New Docx Format (files newer than 2007)")
      .description(
          "Specifies whether this processor will use the newer DOCX format; if set to false, the old .doc format is assumed.")
      .required(false).addValidator(StandardValidators.BOOLEAN_VALIDATOR).defaultValue("true").build();
  ...
  protected boolean newDocxFormat = true;
                                           ...
                                           final String newValue)
  {
    ...
    else if (descriptor.equals(DOCX_FORMAT))
    {
      newDocxFormat = Boolean.parseBoolean(newValue);
...
//          OfficeParser parser = new OfficeParser();
          AbstractOfficeParser parser = newDocxFormat? new OOXMLParser(): new OfficeParser();
...
//          String textData = parserHandler.toString();

          handleText(session,parserMetadata,attributes,parserHandler);
    ...
    catch (Throwable t)
    {
      ...
      localFlowFile = session.putAttribute(localFlowFile,"pg_error", t.getMessage());
